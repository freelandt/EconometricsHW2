...
On Average the GDP per capita grows by \$147.2 per year. See below for model summary and the regression line added to our plot from above. 

```{r}
model1 <- lm(gdpPercap~year, data = filtered.data)
pander(summary(model1))
ggplot(filtered.data, aes(x = year, y = gdpPercap)) + geom_jitter() + geom_smooth(method = )+ ggtitle("Changes in GDP per capita over time") + ylab("GDP Per Capita") + xlab("Year")
```
